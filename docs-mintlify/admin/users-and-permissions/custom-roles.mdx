...
`GET`, `PUT` and `DELETE /api/v1/roles/{roleId}` read, update and remove a role; `PUT` replaces `policies` outright rather than merging, so read the role first if you mean to add a policy. All of these require administrator privileges, and they cover your custom roles only: the built-in roles are not listed or editable through these endpoints.

Assignment is the exception — `PUT /api/v1/roles/{roleId}/users/{userId}` accepts a built-in role's id as well as a custom one. Cube's **Admin** role is not reachable that way: its id returns `404`, so assignment cannot hand out the built-in Admin role. That is a property of assignment alone — `POST /api/v1/roles` accepts a `Global` policy carrying account-wide actions such as `DeploymentsManage` or `AuditLogManage`, so a custom role can grant much of what Admin does. Only an administrator can call any of this.
