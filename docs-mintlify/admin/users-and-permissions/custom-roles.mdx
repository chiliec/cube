...
| Monitoring  | Access chat history                 | View other users' Analytics Chat conversations for the deployment (**Admin → Chats History**).                                                                           |
...
These are the **role builder's** rules, applied in the console before it submits. The API validates differently — see [Managing roles through the API](#managing-roles-through-the-api) below, where a policy with no actions is rejected rather than dropped.
...
Custom roles can also be created, updated and assigned over the [REST API][ref-api-roles], so a team that provisions deployments from CI can grant access to the new deployment in the same pipeline instead of finishing in the console. The API works in the wire format — policies — rather than in the builder's Base Role / Global / Deployment cards, so the mapping is worth reading once before you script against it.
...
<Steps>
  ...
  <Step title="Create a role scoped to it">
    `POST /api/v1/roles`. The Base Role is a `Global` policy naming the tier; the deployment scope is a `Deployment` policy whose `resources` carry that id **as a string**:
    ...
    Use `"resources": ["All"]` to cover every deployment, and `"actions": ["All"]` for the Full access shortcut. The response carries the role's `id`.
  </Step>
...
### How the builder's fields map onto policies

| In the builder | Over the API |
| --- | --- |
| Base Role — Viewer / Explorer / Developer | A `Global` policy whose `actions` include `AIBIView` / `AIBIExplore` / `AIBIDevelop`, with `"resources": ["All"]` |
| Global permissions | Further `actions` on that same `Global` policy |
| A deployment policy card | One `Deployment` policy; **All deployments** is `"resources": ["All"]`, **Specific deployments** is the ids as strings, and **Full access** is `"actions": ["All"]` |

<Warning>

**The Base Role is required in the builder but not by the API, and it is not inferred.** A role sent without a `Global` tier policy is accepted with no error, and a user whose only role is that one holds the deployment grants but has no license tier — they cannot use the product, and nothing says so until they sign in. The auto-bump described above is a rule the builder applies to your input, not one the server applies to yours.

This is deliberate rather than an oversight: permissions stack, so a role that only adds a deployment scope alongside another role carrying the tier is valid. Omit the `Global` policy only when you know another of the user's roles supplies it.

</Warning>

`GET`, `PUT` and `DELETE /api/v1/roles/{roleId}` read, update and remove a role; `PUT` replaces `policies` outright rather than merging, so read the role first if you mean to add a policy. All of these require administrator privileges, and they cover your custom roles only: the built-in roles are not listed, editable or assignable through these endpoints.

<Note>

Deployment ids are checked for shape, not existence: a slug or a name is rejected, but an id whose deployment has since been deleted is kept and simply grants nothing. A role created over the API without a Base Role opens in the builder with that field unset, and Save stays disabled until you pick one.
