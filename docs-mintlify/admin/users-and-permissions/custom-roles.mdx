...
<Steps>
  ...
  <Step title="Create a role scoped to it">
    ...
    `"resources": ["All"]` covers every deployment and `"actions": ["All"]` is the Full access shortcut, as in the builder. Unlike the builder, which drops an incomplete policy on save, the API rejects one with `400` if it grants no action or names no resource. Pick a tier that covers the actions you grant — the [auto-bump rule](#auto-bump-to-developer) applies to your payload too, in the sense that a lower tier here yields a role the builder would not have let you save. The response carries the role's `id`.
  </Step>
