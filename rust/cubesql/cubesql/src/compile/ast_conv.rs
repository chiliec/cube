...
/// followed, when resolving a member and when deciding whether a column can
/// be one alike; the two must agree, or one side would call a column a filter
/// the other cannot restore. It also refuses a self-referencing CTE.
...
/// Upper bound on the nodes one expression walk visits. An expression is at
/// most a whole clause - a group needle holds up to [`MAX_FILTERS`] leaves -
/// so it shares the clause bound; it is named apart so a walk over one
/// expression reads as such.
...
/// identifier quoting reduced, since this API writes `orders_view."status"`
/// where a query may write `orders_view.status`. `drop_qualifiers` also drops
/// the relation name; `normalize_dates` also reduces date literals.
...
/// additions key a clause once rather than once each. A set exists per clause
/// and per way of keying (`ignore_qualifier` is fixed for a query, so that is
/// whether dates are normalized); every materialized set of a clause is
/// extended on append, and all of them are forgotten on any other change.
#[derive(Default)]
struct ClauseKeys {
...
/// plan reports but the query does not spell out the way this API writes it -
/// a merged range, a computed date - has no predicates identifiable one by
/// one, so its column is what is matched. That only holds for a filter the
/// plan reports alone on its member: anything else on the column would go too.
...
/// requested as `2024-01-01` comes back as `2024-01-01T00:00:00.000Z`, and an
/// upper bound this API pads to the end of its day comes back as
/// `2024-12-31T23:59:59.999`. Both are reduced to the date, so that a value
/// compares equal to the date it stands for. Values that don't look like a
/// date are left alone, so that a plain string value is never altered.
...
/// Replaces the filters of the outermost SELECT with the specified set: every
/// filter the query's plan reports is removed from the outermost WHERE and
/// HAVING, then the new filters are added. The plan decides what a filter is,
/// so a predicate extraction cannot represent is never removed and the
/// rewritten query never returns rows the original did not; one the plan
/// reports but the outermost SELECT does not hold - a CTE's, say - stays, and
/// stays reported. The added filters are verified against the plan of the
/// rewritten query.
...
/// Deletes the filters from the outermost SELECT: every equal filter goes, and
/// one that is not there is a no-op. Removal is decided on the AST rather than
/// checked against the rewritten plan, since extraction is plan-wide and a
/// copy in a CTE or a subquery would make that check a false alarm. The
/// rewritten query is still planned, to reject one that does not compile and
/// to report its filters.
...
mod tests {
    ...
    /// The renderer and the drop recurse once per conjunct, and a request is
    /// rendered before it is planned, so a clause at the bound has to survive
    /// both on a worker thread's stack. Tokio gives a worker 2 MiB. The
    /// addition is what brings the clause to the bound: one more is refused.
    #[test]
